...
class NewReminder:
    ...
    def __init__(self, service):
        ...
        nameInput = ctk.CTkEntry(master=nameFrame, placeholder_text="Nom...", border_color=Colors.mainColor)
        nameInput.grid(row=1, sticky="we")
        ...
        hourStartSpinbox = tk.Spinbox(master=periodStartFrame, from_=0, to=23, textvariable=startHour, wrap=True, width=8)
        hourStartSpinbox.grid(row=1, column=0)
        ctk.CTkLabel(master=periodStartFrame, text=" : ").grid(row=1, column=1)
        minuteStartSpinbox = tk.Spinbox(master=periodStartFrame,from_=00, to=59, textvariable=startMin, wrap=True, width=8)
        minuteStartSpinbox.grid(row=1, column=2)
        ...
        hourEndSpinbox = tk.Spinbox(master=periodEndFrame, from_=0, to=23, textvariable=endHour, wrap=True, width=8)
        hourEndSpinbox.grid(row=1, column=0)
        ctk.CTkLabel(master=periodEndFrame, text=" : ").grid(row=1, column=1)
        minuteEndSpinbox = tk.Spinbox(master=periodEndFrame,from_=00, to=59, textvariable=endMin, wrap=True, width=8)
        minuteEndSpinbox.grid(row=1, column=2)
        ...
        countInput = ctk.CTkEntry(master=countFrame, placeholder_text="Nombre...", width=80, border_color=Colors.mainColor)
        countInput.grid(row=1, pady=(0,8))
        ...
        measureTitle = ctk.CTkLabel(master=measureFrame, text="Quantité désirée (en cl)", fg_color="transparent")
        measureTitle.grid(row=0, pady=(4,0))
        measureInput = ctk.CTkEntry(master=measureFrame, placeholder_text="Quantité...", width=80, border_color=Colors.mainColor)
        measureInput.grid(row=1, pady=(0,8))
        ...
        cancelBtn = ctk.CTkButton(master=buttonsFrame, text="Annuler", width=80, height=28, fg_color=Colors.btnColor , hover_color=Colors.hghColor, command=self.root.quit)
        ...
        validateBtn = ctk.CTkButton(master=buttonsFrame, text="Valider", width=80, fg_color=Colors.darkBtnColor, hover_color=Colors.hghColor,
                                    command= lambda: self.service.save(
                                                                nameInput.get(),
                                                                hourStartSpinbox.get(), 
                                                                minuteStartSpinbox.get(), 
                                                                hourEndSpinbox.get(),
                                                                minuteEndSpinbox.get(),
                                                                countInput.get(),
                                                                measureInput.get()
                                                            ))                                         
        validateBtn.pack(side="left", padx=(12,0))

        self.root.protocol("WM_DELETE_WINDOW", self.changeWindow)
        self.root.mainloop()
